...
(defconst clutch-mongodb--aggregation-stage-candidates
  '("$addFields" "$bucket" "$bucketAuto" "$count" "$densify" "$facet"
    "$changeStream" "$changeStreamSplitLargeEvent" "$collStats" "$currentOp"
    "$documents" "$fill" "$geoNear" "$graphLookup" "$group" "$indexStats"
    "$limit" "$listClusterCatalog" "$listLocalSessions" "$listSampledQueries"
    "$listSearchIndexes" "$listSessions" "$lookup" "$match" "$merge" "$out"
    "$planCacheStats" "$project" "$querySettings" "$queryStats" "$rankFusion"
    "$redact" "$replaceRoot" "$replaceWith" "$sample" "$score" "$scoreFusion"
    "$search" "$searchMeta" "$set" "$setWindowFields" "$shardedDataDistribution"
    "$skip" "$sort" "$sortByCount" "$unionWith" "$unset" "$unwind"
    "$vectorSearch")
  "MongoDB aggregation pipeline stage completion candidates.")

(defconst clutch-mongodb--query-operator-candidates
  '("$all" "$and" "$bitsAllClear" "$bitsAllSet" "$bitsAnyClear"
    "$bitsAnySet" "$elemMatch" "$eq" "$exists" "$expr" "$geoIntersects"
    "$geoWithin" "$gt" "$gte" "$in" "$jsonSchema" "$lt" "$lte" "$mod"
    "$ne" "$near" "$nearSphere" "$nin" "$nor" "$not" "$or" "$regex"
    "$size" "$text" "$type" "$where")
  "MongoDB query and projection operator completion candidates.")

(defconst clutch-mongodb--expression-operator-candidates
  '("$abs" "$acos" "$acosh" "$add" "$addToSet" "$allElementsTrue" "$and"
    "$anyElementTrue" "$arrayElemAt" "$arrayToObject" "$asin" "$asinh" "$atan"
    "$atan2" "$atanh" "$avg" "$binarySize" "$bitAnd" "$bitNot" "$bitOr"
    "$bitXor" "$bsonSize" "$ceil" "$cmp" "$concat" "$concatArrays" "$cond"
    "$convert" "$cos" "$cosh" "$covariancePop" "$covarianceSamp"
    "$createObjectId" "$dateAdd" "$dateDiff" "$dateFromParts"
    "$dateFromString" "$dateSubtract" "$dateToParts" "$dateToString"
    "$dateTrunc" "$dayOfMonth" "$dayOfWeek" "$dayOfYear" "$degreesToRadians"
    "$denseRank" "$derivative" "$deserializeEJSON" "$divide" "$documentNumber"
    "$encStrContains" "$encStrEndsWith" "$encStrNormalizedEq"
    "$encStrStartsWith" "$eq" "$exp" "$expMovingAvg" "$filter" "$first"
    "$firstN" "$floor" "$function" "$getField" "$gt" "$gte" "$hash" "$hexHash"
    "$hour" "$ifNull" "$in" "$indexOfArray" "$indexOfBytes" "$indexOfCP"
    "$integral" "$isArray" "$isNumber" "$isoDayOfWeek" "$isoWeek"
    "$isoWeekYear" "$last" "$lastN" "$let" "$linearFill" "$literal" "$ln"
    "$locf" "$log" "$log10" "$lt" "$lte" "$ltrim" "$map" "$max" "$maxN"
    "$mergeObjects" "$meta" "$millisecond" "$min" "$minMaxScaler" "$minN"
    "$minute" "$mod" "$month" "$multiply" "$ne" "$not" "$objectToArray" "$or"
    "$pow" "$push" "$radiansToDegrees" "$rand" "$range" "$rank" "$reduce"
    "$regexFind" "$regexFindAll" "$regexMatch" "$replaceAll" "$replaceOne"
    "$reverseArray" "$round" "$rtrim" "$sampleRate" "$second" "$serializeEJSON"
    "$setDifference" "$setEquals" "$setField" "$setIntersection" "$setIsSubset"
    "$setUnion" "$shift" "$sigmoid" "$similarityCosine" "$similarityDotProduct"
    "$similarityEuclidean" "$sin" "$sinh" "$size" "$slice" "$sortArray"
    "$split" "$sqrt" "$strcasecmp" "$strLenBytes" "$strLenCP" "$substr"
    "$substrBytes" "$substrCP" "$subtract" "$subtype" "$sum" "$switch" "$tan"
    "$tanh" "$toArray" "$toBool" "$toDate" "$toDecimal" "$toDouble"
    "$toHashedIndexKey" "$toInt" "$toLong" "$toLower" "$toObject" "$toObjectId"
    "$toString" "$toUpper" "$toUUID" "$trim" "$trunc" "$tsIncrement"
    "$tsSecond" "$type" "$unsetField" "$week" "$year" "$zip")
  "MongoDB aggregation expression operator completion candidates.")
