...
- Updated the JDBC agent pin to 0.2.9.  The agent now poisons connections whose timed-out driver work does not stop, serializes each metadata session, redacts embedded JDBC URL credentials, validates exact protocol integers, bounds large response cells, and avoids an unconditional liveness round trip before every query.  Disconnected logical connections no longer report live merely because the shared JVM is still running.
- Bounded native MongoDB find results and Redis key discovery/generated collection browsing.  Redis discovery now has both key-count and SCAN-batch limits, with exact key lookup beyond the initial object snapshot.  MongoDB endpoint labels use public client metadata when available while remaining compatible with older clients, and MongoDB/Redis object paths consistently translate protocol failures to `clutch-db-error`.
