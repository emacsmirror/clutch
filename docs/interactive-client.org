...
Tip: short JSON/XML cells render inline with lightweight token faces; long
values show a raw prefix with an ellipsis and can be opened with =v=.
...
connection header.
Sort and paging refreshes also keep manually adjusted column widths when the
result columns are unchanged.

Tip: Press =s= to cycle sort state for the result column at point.  Simple table
results use server-side SQL =ORDER BY=.  UNION, grouped, derived, and other
results that Clutch cannot safely rewrite use a client-side sort of the current
page; the footer marks this scope as =page=.  Use =C= to jump to a different
visible column first, or click its header to cycle that column.  The cycle is
unsorted, ascending, descending, then unsorted again.  Header icons appear after
column names and use the original-size nerd-icons sort glyphs; when nerd-icons
is unavailable, text arrows preserve the sort marker.
...
Single-cell edit buffers use the same staged-mutation vocabulary as result
buffers: =C-c C-c= stages the edited value, and the result buffer's
=C-c C-c= commits all pending row changes.
...
Database =NULL= displays as =<null>= in result cells and edit buffers.  In an
edit buffer, =C-c C-n= sets an explicit =NULL= state; an empty buffer remains an
empty string, and typing =NULL= stages the literal text =NULL=.
...
=clutch-unregister-column-displayer= to remove one registration.  Use the
registration functions or Customize for runtime changes so existing result
buffers invalidate cached cell displays.

Keep displayers pure and cheap: they run on the result rendering path, though
rendered cell strings are cached while the result state is unchanged.  A
displayer should not query the database, read files, start processes, mutate
Clutch state, or schedule asynchronous work.  Return =nil= for values the
function does not handle so the default renderer can display them normally.
If a generic rule depends on SQL type, schema metadata, or backend-specific
state, prefer Clutch's built-in renderer or schema/cache layer over a column
displayer.
...
Insert buffers show every field by default, including generated/defaulted ones.
Field labels are read-only, aligned to a shared value column, and may show tags
such as =generated=, =default=..., =enum=, =bool=, =json=, and =required=.  The
active field line is highlighted.  Insert buffers also validate locally as you
type: obvious enum / bool / json / numeric / date-time mistakes are shown inline
on the current field before staging.
...
The copy transient (=c=) shows =Refine selection (No|Yes)= on =-r=.  With an
active region, enabling refinement before choosing a format lets you interactively
...
duration of the session.  The refine switch can be toggled before a region is
active; choosing a copy format with refinement still requires a region and
...
The Record transient labels =RET= from the field at point: =Follow FK=,
=Expand=, =Collapse=, or =Show value=.  It is inapt when point is not on a
field.
...
PostgreSQL, SQLite, and JDBC backends execute staged DML through parameterized
prepared/bound values under the hood; only the preview renders readable SQL
literals.
...
the primary set is =TABLE= / =VIEW= / =SYNONYM= / =COLLECTION= / =KEY=,
configurable via =clutch-primary-object-types=.  The picker
...
Finite state is displayed as parenthesized choices, with the active choice
highlighted.  For example, the connection menu shows
=Auto-commit (manual|auto)=, and the result menu shows current filter, sort,
copy-refinement, and layout states.
...
In the result buffer, the *Staged* heading includes the current pending count.
Its copy/save actions and the mutation group's commit/discard actions are inapt
when no changes are staged.  The unified staged-commit workflow uses =i= to
stage an insert, =d= to stage a deletion, =C-c C-k= to discard a pending
change at point, and =C-c C-c= to commit all pending changes
(INSERT → UPDATE → DELETE) in a single confirmation.
...
- =clutch-jdbc-fetch-size= controls JDBC cursor batch size from 1 through
  10,000 rows (default 500).
