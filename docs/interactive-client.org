...
Tip: Press =s= to cycle server-side SQL sort state for the result column at
point.  Use =C= to jump to a different visible column first, or click its
header to cycle that column.  The cycle is unsorted, ascending, descending, then
unsorted again.  Header icons appear after column names and use the
original-size nerd-icons sort glyphs; when nerd-icons is unavailable, the sort
marker is omitted.
