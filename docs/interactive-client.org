...
=M-.= through the buffer-local xref backend.  CAPF completion handles SQL
identifiers, including empty column positions such as =select | from users=.
=TAB= completes columns after a table or alias dot, then falls back to
indentation elsewhere; other completion stays on normal CAPF keys such as
=M-TAB= inherited from Emacs.
...
picker.  For SQLite files, use the query-console path instead of opening the
database file itself as an editor buffer.
