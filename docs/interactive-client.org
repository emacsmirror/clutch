...
| =C-c TAB=   | Complete SQL identifiers at point, including empty column positions |
| =C-c ?=     | Transient menu             |

Standard editing integrations still apply in =clutch-mode=: xref lookup uses
=M-.= through the buffer-local xref backend.  =TAB= completes columns after
a table or alias dot, then falls back to indentation elsewhere; other
completion stays on normal CAPF keys such as =M-TAB= inherited from Emacs.
