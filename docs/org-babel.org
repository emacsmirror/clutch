...
Execute SQL directly in org files via the separate [[https://github.com/LuciusChen/ob-clutch][ob-clutch]] package.  It bridges clutch connections into Org-Babel block types for MySQL, PostgreSQL, SQLite, and the generic =clutch= backend used for JDBC databases.
...
The generic =clutch= block does not need an inline =:backend= when =:connection= is present; the backend is taken from the saved connection entry.  An explicit =:backend= is only required for inline connection parameters.
...
Compatibility entry files live in the [[https://github.com/LuciusChen/ob-clutch][ob-clutch]] package:
...
| =:tramp-default-directory= | ssh-like TRAMP directory for Clutch TRAMP forwarding                 |

** Connection Caching

Connections are cached by parameter hash and source TRAMP origin, then reused across source block evaluations in the same Emacs session.  All cached connections are cleanly disconnected when Emacs exits (via =kill-emacs-hook=).
