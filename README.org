...
JDBC support ships inside =clutch= itself, but the runtime still needs Java 17+ and =clutch-jdbc-agent.jar=. Individual JDBC databases may also need driver jars. The current development version pins JDBC agent 0.2.11 and verifies the published jar before starting it. JDBC staged mutations use prepared values. Idle-timeout and Oracle =ORA-12592= metadata failures recover without replacing the foreground session, while a failed retry or schema restore is invalidated rather than reused. If a fatal foreground failure removes the primary JDBC session, Clutch reports that original failure without replaying the SQL and automatically reconnects the preserved console on the next command. For Org-Babel support, install the separate [[https://github.com/LuciusChen/ob-clutch][ob-clutch]] package.
