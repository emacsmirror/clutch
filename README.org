...
JDBC support ships inside =clutch= itself, but the runtime still needs Java 17+ and =clutch-jdbc-agent.jar=.  Individual JDBC databases may also need driver jars.  The current development version pins JDBC agent 0.2.9 and verifies the published jar before starting it.  JDBC staged mutations use prepared values, and an idle-timed-out metadata session is recovered without replacing the foreground session. For Org-Babel support, install the separate [[https://github.com/LuciusChen/ob-clutch][ob-clutch]] package.
